 ...
 */
function upload_sale_sign_pic(DiyCarmerFinishChu_name, DiyCarmerFinishChu_url) {
	...
	oUseTool.INpicfile(thispicguid, DiyCarmerFinishChu_name, DiyCarmerFinishChu_url, todaytime, "", uploadinserttime, function(states){
		...
		console.log("插入图片成功");
		if(states){
			upload_topicdbonlineNewReport(thispicguid, todaytime, DiyCarmerFinishChu_name, DiyCarmerFinishChu_url);
		}
	});
	return;
	//将图片保存到本地得操作
	...
	$kunchi.kunchiuploadpic('api/Upload', JSON.stringify(piclist), function(ret, err) {
		...
		if (ret) {
			...
			upload_topicdbonlineNewReport(todaytime, DiyCarmerFinishChu_name, DiyCarmerFinishChu_url);
...
function upload_topicdbonlineNewReport(localguid, savetimefuc, DiyCarmerFinishChu_name, DiyCarmerFinishChu_url) {
	...
	oUseTool.INpicbind(thispicguid, cPicPath, cPicClass, storeid, "PhotoType", $api.getStorage("id"), $api.getStorage("roleName"), $api.getStorage("realName"), guid, $kunchi.todaytime(), localguid, function(states){
		...
		console.log("插入绑定成功");
		if(states){
			showPicToHmtl(DiyCarmerFinishChu_name, 'data:image/jpg;base64,'+DiyCarmerFinishChu_url, thispicguid);
		}
	});
	return;
	//将图片关系保存到本地
	...
	$kunchi.kunchipost('ActionApi/T_Pics/T_PicsAdds', picsaddlist, function(ret, err) {
		...
		// console.log(JSON.stringify(ret) + "==" + JSON.stringify(err));
		if (ret) {
			showPicToHmtl(DiyCarmerFinishChu_name, 'data:image/jpg;base64,'+DiyCarmerFinishChu_url, ret[0].id);
		} else {
			api.confirm({
				title : '提示',
				msg : '上传图片失败,是否重试?',
...
function postdelpiconline(motherid, onlineid) {
    ...
    if(onlineid.length == 36){
    	oUseTool.DELpicall(onlineid, function(states){
			delxml(motherid);
		});
		return;
    }
