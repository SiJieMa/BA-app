...
function initpandian(tag) {
	...
	function initgoodlist(ret) {
		var newret = ret.map(function (value) {
			var kucun = value.iCurrentInventory == 0 ? "0" : "";
			var object = {
...
function uploadPanDian(tag) {
	...
	var isturnhistory = 1; //0跳转异常详情  1跳转历史列表
	var isallowtijiao = 0; //0允许提交  1不允许提交
	var uploadlist = tag.goodlist.forEach(function (value) {
		console.log(value.kucun.trim().length + "===" + value.kucun);
		if (value.iCurrentInventory == 0 && value.kucun == 0) {} else {
