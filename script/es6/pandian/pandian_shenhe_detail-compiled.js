...
function initlist(tag) {
	...
	var posturl = 'ActionApi/T_Inventory/GetInventoryCheckDetailByAuthorize';
	$kunchi.kunchipost(posturl, obj, function (ret, err) {
		...
		api.hideProgress();
		if (ret) {
			var AuthorListlength = ret[0].AuthorList.length - 1;
			...
			tag.list = ret.map(function (value, index) {

				if (index == 0) {

					var newsplist = [];
					for (var i = value.AuthorList.length - 1; i >= 0; i--) {
						var spvalue = value.AuthorList[i];
						var dtCreateTimeYear = spvalue.dtCreateTime.split("T")[0];
						var dtCreateTimeMonth = spvalue.dtCreateTime.split("T")[1];
						var dtCreateTimeMonthnew = dtCreateTimeMonth.split(".")[0];
						spvalue.dtCreateTime = dtCreateTimeYear + " " + dtCreateTimeMonthnew;

						var iStatue = spvalue.iStatue;
						var cAuthorName = spvalue.cAuthorName;
...
function shenhepandian(tag, statue) {
	var tishimsg = statue == 1 ? '确定要审批通过盘点?' : '确定要拒绝盘点?';
	vant.Dialog.confirm({
		title: '提示',
		message: tishimsg
	}).then(function () {
		...
		var userrole = $api.getStorage("roleName");
		...
		obj.statue = statue;
		console.log(JSON.stringify(obj));
		var posturl = 'ActionApi/T_Inventory/SetAuthorizeInventoryCheck';
		$kunchi.kunchipost(posturl, obj, function (ret, err) {
			console.log(JSON.stringify(ret) + "----" + JSON.stringify(err));
			api.hideProgress();
			if (ret) {
