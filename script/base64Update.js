...
function ChuLiNewCarmerPic(imagepath){
	...
	putpic(DiyCarmer_item1, $api.getStorage("realName"), DiyCarmer_item2, DiyCarmer_item3, paitime, imagepath, function(isrue, picurl){
		if(isrue == "yes"){
			...
			if(DiyCarmer_item3 == '汉高零售拍照'){
				DiyCarmerFinishChu_name = 'HGLS'+myDate.getTime()+$api.getStorage("username")+".jpg";
			}
			DiyCarmerFinishChu_url = picurl;

			if((DiyCarmer_item3 == '促销开店照')||(DiyCarmer_item3 == '周末促POS照')){
				api.sendEvent({
				    name: 'PaiPicFinish',
				    extra: {
				        PaiPicFinish: 'ok'
			...
			if((DiyCarmer_item3 == '进店签到')||(DiyCarmer_item3 == '离店签到')||(DiyCarmer_item3 == '汉高零售拍照')){
				upload_sale_sign_pic();
			}else{
				setTimeout("InsterIntoDataBase()",300);
			}
		}else{
			DiyCarmerFinishChu_name = myDate.getTime()+$api.getStorage("username")+".jpg";
			if(DiyCarmer_item3 == '汉高零售拍照'){
...
function InsterIntoDataBase(){
	...
	var sql = 'INSERT INTO piclist (picurl, picname, isupload, filepath) VALUES ("' + DiyCarmerFinishChu_url + '", "' + DiyCarmerFinishChu_name + '", "0", "'+todaytime+'")';
	// console.log(sql);
	executeSql(sql, function(num) {
		if (num == 1) {
			//console.log('走了数据库:' + DiyCarmer_item3);
			api.sendEvent({
			    name: 'PaiPicFinish',
			    extra: {
			        PaiPicFinish: 'ok'
