...
//		console.log(JSON.stringify(ret) + "==" + JSON.stringify(err));
		...
		if(ret){
			...
			if(ret.length != 0){
				...
				for(var a = 0; a < ret.length; a++){
					if(isshowbtn == 0){
						var li = '<li id="'+ret[a].iRorptId+'li">'
							+		'<span tabmode="huise" class="title" tabmode="huise">'+ret[a].cStoreFullName+'</span>'
							+	'</li>';
						$api.append(el, li);
					}else{
						var li = '<li id="'+ret[a].iRorptId+'li">'
							+		'<span tabmode="huise" class="title" tabmode="huise">'+ret[a].cStoreFullName+'</span>'
							+		'<span onclick="showmenucaozuoshan(this)" iRorptId="'+ret[a].iRorptId+'" class="caozuo">〈操作</span>'
							+	'</li>';
						$api.append(el, li);
					}
				}
...
function show_all_worklist(){
	...
	for(var a = 0; a < zidianvalue.length; a++){
		var html = '<li iReportType="'+zidianvalue[a].id+'" iStoreId="" value="'+canshunum+'" id="li'+canshunum+'" onclick="changestatues(this);">'
				+		'<i id="fa'+canshunum+'" class="fa fa-circle-o" aria-hidden="true"></i>'
				+		'<span id="'+canshunum+'" codemendian="nodaihao" class="worktype">'+zidianvalue[a].cDictName+'</span>'
	...
	for(var a = 0; a < saleshopba.length; a++){
		var html = '<li iReportType="0" iStoreId="'+saleshopba[a].iStoreId+'" id="li'+canshunum+'" value="'+canshunum+'" onclick="changestatues(this);">'
				+		'<i id="fa'+canshunum+'" class="fa fa-circle-o" aria-hidden="true"></i>'
				+		'<span id="'+canshunum+'" codemendian="'+saleshopba[a].cStoreCode+'" class="worktype">'+saleshopba[a].cStoreFullName+'</span>'
				+		'<span class="daihao">'+saleshopba[a].cStoreCode+'</span>'
				+	'</li>';
		$api.append(el, html);
		canshunum++;
	}
			...
			if(ret.length != 0){
				...
				for(var a = 0; a < ret.length; a++){
					...
					if(ret[a].iWorkType == 0){
						jihuawaihtml = "";
						var li = '<li id="'+ret[a].iRorptId+'" class="mui-table-view-cell">'
							+			'<div cCustomerFullName="'+ret[a].cCustomerFullName+'" iRorptListTime="'+ret[a].dtPlantTime.split("T")[0]+'" cStoreCode="'+cstorecode+'" iStoreId="'+ret[a].iStoreId+'" cLon="'+ret[a].cLon+'" cLat="'+ret[a].cLat+'" iRorptId="'+ret[a].iRorptId+'" cStoreFullName="'+ret[a].cStoreFullName+'" hasFlag="'+hasFlag+'" iPlantFlag="'+iPlantFlag+'" onclick="opendetial(this)" class="mui-slider-handle">'
							+				'<span class="namelist">'+jihuawaihtml+ret[a].cStoreFullName+'<br/>门店编码:<i class="mendiancode">'+cstorecode+'</i></span>'
							+				'<span class="stauteslist">'+status+'</span>'
							+			'</div>'
							+		'</li>';
